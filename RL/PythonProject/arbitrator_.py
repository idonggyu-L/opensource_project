...
class MetaControllerSoftSelect:
    ...
    def run_episode(self, max_steps=500):
        ...
        while not done and step < max_steps:
            ...
            probs = torch.softmax(logits, dim=-1).detach().cpu().numpy()[0]  # (n_agents,)

            actions_np = np.array(actions, dtype=np.float32).reshape(self.n_agents, -1)
            a_sel = np.sum(probs[:, None] * actions_np, axis=0)
            ...
            self.buf_act.append(probs)   # soft label ??
    ...
    def _finish_meta_update(self, last_value=0.0):
        ...
        act_batch  = torch.stack([torch.as_tensor(p, dtype=torch.float32, device=self.device)
                                  for p in self.buf_act])   # (B, n_agents)

        logits = self.meta_pi(obs_batch, chis_batch)
        log_probs = torch.log_softmax(logits, dim=-1)  # (B, n_agents)

        # cross entropy with soft labels
        policy_loss = -(act_batch * log_probs).sum(dim=-1)
        policy_loss = (policy_loss * adv_t).mean()
